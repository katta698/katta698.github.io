...
git add blog/ posts/ scripts/ _templates/ intelligence/ how-this-was-made/ index.html resume.html now.html sw.js
git commit
...
2. `git add blog/ posts/ scripts/ _templates/ intelligence/ how-this-was-made/ index.html resume.html now.html sw.js`
   ...
   **`how-this-was-made/` is on this line for the same reason, and was
   missing until 2026-09-23.** `sync_blog.py` regenerates the colophon on
   every run, so every window rebuilt it and then left it unstaged. The
   counts on the live page froze at whenever the `main` window last
   committed it by hand: the page read 279 posts while `origin/main`
   carried 281. The page's own promise is that "every number below is
   counted from the repository when this page is built" -- which stayed
   true, and stopped meaning anything, because the build never shipped.
