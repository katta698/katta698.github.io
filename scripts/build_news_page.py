...
.nav-logo{display:flex;align-items:center;gap:.65rem;text-decoration:none;color:#EDEBE6}
  ...
  /* Hero video, matching the portfolio and the blog.
     The header already held the heading, so the clip goes behind what is there
     rather than replacing it. min-height is what gives it room: without it the
     hero is sized by its own text and the video crops to a letterbox. Both
     layers are pointer-events:none so neither can swallow a click. Copied from
     blog.css rather than reinvented, so all five pages crop and tint alike. */
  header.hero{position:relative;overflow:hidden;
    min-height:clamp(260px,34vh,380px);display:flex;flex-direction:column;
    justify-content:center}
  .hero-video{position:absolute;inset:0;width:100%;height:100%;
    object-fit:cover;z-index:0;pointer-events:none}
  .hero-overlay{position:absolute;inset:0;z-index:1;pointer-events:none;
    background:linear-gradient(180deg,rgba(29,35,34,.45) 0%,rgba(29,35,34,.88) 100%)}
  body.light .hero-overlay{
    background:linear-gradient(180deg,rgba(29,35,34,.35) 0%,rgba(29,35,34,.75) 100%)}
  header.hero > *:not(.hero-video):not(.hero-overlay){position:relative;z-index:2}
  /* Reduce Motion stops the clip; the poster still shows. Motion nobody asked
     for is an accessibility problem, not a taste one. */
  ...
  gap:.35rem 1.15rem;line-height:1.6}
.lede-links .lede-go{color:var(--accent);font-weight:600;white-space:nowrap}
/* The separator disappears when the two wrap onto separate lines, where a
   middot floating at the end of a line reads as a typo rather than a divider. */
.lede-sep{opacity:.45}
@media (max-width:560px){ .lede-sep{display:none} .lede-links{gap:.5rem 0;flex-direction:column;align-items:flex-start} }
...
/* A 44px thumb target without a 44px block of paint. The button is 34px on a
   phone, already past the 24px WCAG 2.5.8 floor, and growing it would put
   more opaque square over the body text it floats above. The touch area is
   extended past the edge instead: invisible, no layout, nothing moves. */
...
.nav-logo .brand-mark{width:30px;height:30px;border-radius:50%;
  ...
  background:var(--nav-mark, rgb(41,35,35))} /* --nav-mark, not this page's own tokens: site-footer.js pins it so the circle matches the other four pages. */
...
<script src="/blog/assets/occasion-banner.js?v=__JSV__" data-occasion-banner></script>
...
<header class="hero">
  ...
  <div class="inner">
    ...
    <!-- The two links sat at the end of the sentence above, 4px apart, in the
         ...
         onto a sentence about filtering. They are two different actions: one
         leaves for another page, one moves down this one, which is what the
         arrows say. On their own line with room between them, the arrows are
...
/* The filters live in the URL, so that leaving the page does not clear them.
 *
 * Reported as: filter to AWS, last 7 days, one service, open an announcement,
 * come back -- and it is showing all clouds and 30 days again. Every row links
 * out with target="_blank", so "coming back" can mean a tab the browser threw
 * away and reloaded while the reader was reading. State held only in these
 * five variables cannot survive that; state held in the URL does, whether the
 * page comes back from the back/forward cache or is reloaded from scratch.
 *
 * It also makes a filtered view something you can send to somebody:
 * /intelligence/whats-new/?cloud=aws&days=7 is a link to what AWS shipped this
 * week, not to a page somebody else then has to filter themselves.
 *
 * replaceState rather than pushState, for the same reason as the events page:
 * tapping four filters should not put four entries in the back stack for a
 * reader to walk out through.
 *
 * `shown` is deliberately NOT carried. Restoring a reader to row 300 means
 * building 300 rows before first paint, which is the cost this page was
 * rebuilt to avoid; the back/forward cache restores scroll position when it
 * can, and that is the right mechanism for it.
 */
var DAY_OPTS = [7, 30, 90, 0];
function toUrl(){
  var p = [], qv = document.getElementById('q').value.trim();
  if(cloud !== 'all') p.push('cloud=' + encodeURIComponent(cloud));
  if(days !== 30) p.push('days=' + days);
  if(svc) p.push('svc=' + encodeURIComponent(svc));
  if(qv) p.push('q=' + encodeURIComponent(qv));
  if(more) p.push('view=more');
  var url = location.pathname + (p.length ? '?' + p.join('&') : '');
  try{ history.replaceState(null, '', url + location.hash); }catch(e){}
}
/* Returns true when the blog and bulletin records are wanted, which the caller
   has to fetch before it can show them. */
function fromUrl(){
  var p;
  try{ p = new URLSearchParams(location.search); }catch(e){ return false; }
  // Only values this page actually offers. A hand-edited ?cloud=oracle or
  // ?days=4000 should land on the default view, not on an empty one.
  var c = p.get('cloud');
  if(c && ['all','aws','azure','gcp'].indexOf(c) !== -1) cloud = c;
  var d = p.get('days');
  if(d !== null && DAY_OPTS.indexOf(parseInt(d, 10)) !== -1)
    days = parseInt(d, 10);
  var s = p.get('svc');
  if(s) svc = s;
  var q = p.get('q');
  if(q) document.getElementById('q').value = q;
  return p.get('view') === 'more';
}
...
document.addEventListener('click', function(e){
  ...
  if(b.dataset.moreSrc !== undefined){
    ...
    if(want && !MORE){
      ...
      fetch('/intelligence/news-more.json').then(function(r){return r.json();})
        ...
        .catch(function(){ MORE = []; })
        .then(function(){ moreLoading = false; shown = 60;
                          buildPills(); render(); toUrl(); });
      return;
    }
    more = want; shown = 60; buildPills(); render(); toUrl();
    return;
  }
  ...
  shown = 60; buildPills(); render(); toUrl();
});
...
fetch('/intelligence/news.json').then(function(r){return r.json();}).then(function(d){
  DATA = d.items || [];
  var wantMore = fromUrl();
  // A restored ?svc= is only honoured if the data actually carries that
  // service. Services come and go from the feeds, and a stale link should
  // show the cloud it asked for rather than an empty list with a pill on it.
  if(svc && !DATA.some(function(r){ return r.s.indexOf(svc) !== -1; })){
    if(!wantMore) svc = '';
  }
  if(wantMore){
    // Restoring the blog and bulletin view means the 1.2 MB record set has to
    // be here before anything is rendered, or the reader gets their filters
    // back over the wrong pool and watches the list change under them.
    moreLoading = true; buildPills(); render();
    fetch('/intelligence/news-more.json').then(function(r){return r.json();})
      .then(function(d2){ MORE = d2.items || []; more = true; })
      .catch(function(){ MORE = []; })
      .then(function(){
        moreLoading = false;
        if(svc && !pool().some(function(r){ return r.s.indexOf(svc) !== -1; }))
          svc = '';
        buildPills(); render(); toUrl();
      });
    return;
