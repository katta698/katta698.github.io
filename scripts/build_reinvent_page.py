...
<main class="page">
 ...
 <section class="resultbar">
  ...
  <div class="viewtabs" role="tablist">
   ...
   <button id="tab-plan2" class="vt" role="tab" aria-selected="false">Plan a day</button>
 ...
 <section id="plan" class="plan" hidden>
  <p class="notebook">Your plan and your notes live <b>in this browser
   ...
   it private and makes it work with no signal, but it also means a cleared
   browser takes them with it, and another device will not see them. Use
   <b>Back up notebook</b> below before the event and after each day; the
   file it writes restores everything, including the notes.</p>
  <div id="planbody"></div>
  <div class="planfoot">
 ...
 <section id="plan2" class="planwrap" hidden>
  ...
  <div class="maprow plrow">
   <label>Day <select id="pl-day"></select></label>
   <label>Starting from <select id="pl-at"></select></label>
   <label>Focus <select id="pl-lane"></select></label>
   <label>Service <select id="pl-service"></select></label>
   <label>Pace
    <select id="pl-pace">
   ...
   <label class="chk"><input id="pl-sponsored" type="checkbox">
    include sponsored sessions</label>
   <!-- Reset sits WITH the controls. It used to live at the foot of the
        itinerary, which meant scrolling past six or seven sessions to
        clear a form whose every input is up here. -->
...
#pl-service{max-width:min(100%, 22rem)}
...
.saved{position:absolute; right:10px; bottom:8px; font-size:11.5px;
  color:var(--accent2); opacity:0; transition:opacity .2s;
  pointer-events:none}
.saved.on{opacity:1}
