...
(function () {
  ...
  function browseTip(list) {
    ...
    if (state.day) {
      ...
      bits.push("Realistically about <b>five to seven</b> of them fit into "
                + "one day once travel between buildings and a break are "
                + "honoured.");
    } else if (state.venue) {
  ...
  function advise(day, startVenue, chain) {
    ...
    tips.push({ k: "ok", t: chain.length + " session"
      ...
      + " to " + hhmm(last.e),
      d: "That is a full day. Across this catalog five to seven "
       + "substantial sessions is what actually fits once travel and a "
       + "break are honoured — the theoretical maximum is around "
       + "sixteen, but only by chaining 20-minute sponsored talks in one "
       + "room with no gap at all." });
