...
(function sample() {
  ...
  /* ...and not before the fonts are in either.
     Same reason, one layer further down: the bar's width depends on the
     wordmark's, and the wordmark is 114.8px in the fallback serif against
     96.6px in Playfair. Under the full preflight run this reported the
     portfolio's nav-links sliding 822 -> 806 and passed three times in a row
     alone -- an 18px font swap, not a reservation holding the wrong space.
     document.fonts.status covers the case where there are no webfonts at
     all, which reads 'loaded' immediately. */
