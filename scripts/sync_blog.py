...
def write_colophon():
    """Rebuild /how-this-was-made/, whose every figure is counted at build.

    Called from here for the same reason write_sitemap() is: that page counts
    the posts, the pages, the checks and the scheduled jobs at build time and
    prints them as facts. Left to be run by hand it would say 253 posts a
    fortnight after it said something true, and the one claim it makes about
    itself -- that nothing on it is typed in -- would be the first thing
    wrong with it.
    """
    import subprocess
    try:
        out = subprocess.run(
            [sys.executable, str(REPO_ROOT / "scripts" / "build_colophon.py")],
            capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=300,
            encoding="utf-8", errors="replace")
        for line in (out.stdout or "").strip().splitlines()[-2:]:
            print(line)
    except Exception as exc:                                    # noqa: BLE001
        print(f"  colophon not regenerated ({str(exc)[:50]})")
...
(function(){{
  ...
  form.addEventListener('submit',function(e){{
    ...
    fetch(API_URL,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{question:q}})}})
      .then(function(r){{
        if(!r.ok){{var he=new Error('HTTP '+r.status);he.http=r.status;throw he;}}
        return r.json();
      }})
      .then(function(data){{
      ...
      .catch(function(err){{
        // "Check your connection" for a 500 is a lie that costs a bug report.
        // Asked "Latest posts" on a desktop, got a connection error, and the
        // connection was fine -- the Lambda had raised a TypeError building
        // its answer and the blanket catch here relabelled it as the reader's
        // fault. Say which side failed, so the next one is reported as what it
        // is rather than shrugged at.
        var http = err && err.http;
        output.innerHTML = http
          ? '<p class="ask-error">Something went wrong at my end (HTTP ' + http +
            '). Not your connection — try again, or ask me another way.</p>'
          : '<p class="ask-error">Could not reach the server — check your connection and try again.</p>';
      }})
...
(function(){{
  ...
  fetch(SUMMARY_API_URL + '?slug=' + encodeURIComponent(slug))
    .then(function(r){{
      if (r.status === 404) {{ var e = new Error('no summary generated yet'); e.pending = true; throw e; }}
    ...
    .then(function(data){{
      ...
      if (!rows.length) throw new Error('summary record has no overview, key detail or takeaway');
    ...
    .catch(function(err){{
      // Never swallow this. An empty catch here meant a 404 on a brand-new post,
      // a dead endpoint and a CORS failure all rendered identically as "the
      // section does not exist" -- with nothing in the console, on every page,
      // indefinitely. It had to be found by eye and then diagnosed by querying
      // the API by hand.
      var pending = !!(err && err.pending);
      var detail = (err && err.message) || 'unknown error';
      var msg = '[at-a-glance] ' + (pending ? 'pending' : 'FAILED') +
                ' for "' + slug + '": ' + detail;
      if (pending) {{ console.warn(msg); }} else {{ console.error(msg); }}
      mount.innerHTML =
        '<div class="quick-summary-status" style="font-size:12px;color:var(--text-muted);' +
        'padding:8px 0;border-top:0.5px solid var(--border)">' +
        esc(pending
              ? 'At a glance appears within a few hours of publishing.'
              : 'At a glance could not be loaded: ' + detail) +
        '</div>';
    }});
...
<!-- A click on the audio control before its script exists must not vanish.
     ...
     threw ReferenceError and did nothing, silently: press once, nothing
     happens, press again and it works. Reported as "the music button takes a
     couple of clicks to open".
     So the name exists from the first frame, and all it does is remember that
     it was asked. The real implementation replaces it and, if a click was
     already banked, acts on it immediately -- so the first press is honoured
     late rather than lost. -->
<script>
window.__audioWanted = false;
window.toggleBlogAudio = function () {{ window.__audioWanted = !window.__audioWanted; }};
</script>
<!-- Before first paint, not injected after load.
     ...
     790ms in -- long after the page looked finished and settled. A
     click aimed at a nav link in that window lands where the link no
     longer is, which is one of the ways "I have to click a couple of
     times" happens. Blocking and ~10KB, against a header that stops
     moving under the reader.
     data-occasion-banner is the marker blog.js and site-footer.js
     both guard on, so neither injects a second copy. -->
<script src="/blog/assets/occasion-banner.js?v={JS_VERSION}" data-occasion-banner></script>
...
def build_post_page(post, prev_post, next_post):
    ...
    # A roundup is the EDITED view of a week: ranked, with an opinion. The
    # announcement browser is the unedited one. A reader who has just finished
    # "here is what shipped this week" is one thought away from "but what about
    # my service", so the link belongs here more than anywhere else on the site
    # -- and it is the only thing that makes /intelligence/ reachable at all,
    # which it was not: nothing on the home page or the blog linked to it.
    whats_new_html = ""
    if any("Weekly Intelligence" in t for t in post.get("labels", []) or tags):
        whats_new_html = (
            '<div class="wn-followon">'
            '<div class="wn-followon-title">Looking for one service rather '
            'than a whole week?</div>'
            '<p>Every AWS, Azure and Google Cloud announcement is browsable by '
            'service and date, each linked to the vendor&rsquo;s own page &mdash; '
            'and the other <a href="/intelligence/">weekly roundups</a> are '
            'collected in one place.</p>'
            '<a class="wn-followon-btn" href="/intelligence/whats-new/">'
            'Browse announcements &rarr;</a>'
            '</div>')
...
{cloud_rows}
    ...
    _gcplab_feed = [
        _feed_item(p, _week_num(p), _re.sub(r'^Week \d+\s*[-–—]\s*', '', p["title"]))
        for p in sorted(gcplab_posts, key=_week_num, reverse=True)[:3]
    ]
...
(function () {{
  ...
  .then(function (d) {{
    ...
    // met it mid-word.
    // Know the speed BEFORE the crawl starts.
    //
    // This used to start the animation here and correct its duration from an
    // idle callback up to three seconds later. The stylesheet's 60s is a
    // duration, not a speed: the track is the content twice over, about
    // 29,800px on the blog index, so 60s means 249px/s -- and the corrected
    // value is 60px/s. For the first seconds of every single load the ticker
    // therefore ran at four times its own speed and then dropped.
    //
    // Reported as "it's scrolling too fast, and then it's slowing down", from
    // a phone, where the window is 294px wide and the text whips past. The
    // comment below it said that was "a speed, not a failure". It was a
    // failure; it just was not a broken one.
    //
    // scrollWidth forces a layout, which is what the idle callback was
    // avoiding. One reflow of a forty-item inline-flex costs less than four
    // seconds of visibly wrong motion on every visit.
    var setSpeed = function () {{
      var w = track.scrollWidth / 2;
      // Math.round(w/60) is 0 under 30px, and animation-duration:0s does not
      // mean slow -- it means the animation completes instantly and sits
      // frozen at its end state, which is indistinguishable from a ticker that
      // has failed.
      if (w > 120) {{
    ...
    if (setSpeed()) {{
      ...
      // Not measurable yet: wait a frame rather than start at the wrong speed.
    ...
    // A late re-measure, for when a webfont lands and the track changes width.
    // Same function, so the two cannot disagree about the speed.
    var measure = setSpeed;
...
/* Was it pressed before this file existed? The stub in the head banks that,
   and this is the moment it can be acted on. Without this the first press is
   simply dropped, which is the bug the stub exists to fix -- a placeholder
   that swallows the click as quietly as the ReferenceError did would be no
   better. */
if (window.__audioWanted) {{
  window.__audioWanted = false;
  toggleBlogAudio();
}}
</script>
